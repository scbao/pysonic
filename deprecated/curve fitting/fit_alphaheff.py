...
# @Last Modified time: 2017-02-14 15:48:21

''' Detailed fitting strategy of the alpha_h_eff profiles '''
...
if not lookup_filepaths:
    ...
    print('importing alphah_eff profiles from lookup tables')
    nfiles = len(lookup_filepaths)

    # Initialize coefficients matrices
    amps = np.empty(nfiles)
    alphah_eff = np.empty((nfiles, nQ))
    ...
    for i in range(nfiles):
        ...
        if not mo:
            ...
            with open(lookup_filepaths[i], 'rb') as fh:
                ...
                Qm = lookup['Q']
                alphah_eff[i, :] = lookup['alpha_h_eff']
    ...
    if baseline_ind == -1:
        ...
        alphah_eff_sub = (alphah_eff - alphah_eff[baseline_ind, :])

        # Peaks fitting on even profiles
        print('fitting exponential law to profiles peaks')
        alphah_eff_sub_peaks = np.amax(alphah_eff_sub, axis=1)
        popt, _ = curve_fit(Exp0, amps, alphah_eff_sub_peaks, p0=(1.8e14, 3e-5))
        alphah_eff_sub_peaks_fit = Exp0(amps, *popt)

        # Normalization
        print('normalizing subtracted profiles')
        alphah_eff_sub_norm = alphah_eff_sub[1:, :]\
            / alphah_eff_sub_peaks[1:].reshape(namps - 1, 1)

        # Normalized profiles fitting
        print('fitting hybrid gaussian-exp law to normalized alphaheff-sub')
        alphah_eff_sub_norm_fit = np.empty((namps - 1, nQ))
        params = np.empty((namps - 1, 6))
        for i in range(namps - 1):
            print(i)
            popt, _ = curve_fit(dualGauss, Qm, alphah_eff_sub_norm[i],
                                bounds=([-np.infty, -np.infty, 0., 0., 0., 0.],
                                        [0., 0., np.infty, np.infty, np.infty, np.infty]),
                                max_nfev=100000)
            alphah_eff_sub_norm_fit[i, :] = dualGauss(Qm, *popt)
            params[i, :] = np.asarray(popt)



        # Predict alphah_eff profiles
        print('predicting alphah_eff by reconstructing from fits')
        alphah_eff_sub_predict = np.vstack((np.zeros(nQ), alphah_eff_sub_norm_fit))\
            * alphah_eff_sub_peaks_fit.reshape(namps, 1)
        alphah_eff_predict = alphah_eff_sub_predict + alphah_eff[baseline_ind, :]

        # Analyze prediction accuracy, in wide and realistic charge ranges
        alphah_eff_trueQ = alphah_eff[:, i_trueQ_lb:i_trueQ_ub]
        alphah_eff_predict_trueQ = alphah_eff_predict[:, i_trueQ_lb:i_trueQ_ub]
        alphah_eff_diff = alphah_eff_predict - alphah_eff
        alphah_eff_diff_trueQ = alphah_eff_diff[:, i_trueQ_lb:i_trueQ_ub]
        alphah_eff_maxdiff = np.amax(np.abs(alphah_eff_diff), axis=1)
        alphah_eff_maxdiff_trueQ = np.amax(np.abs(alphah_eff_diff_trueQ), axis=1)
        alphah_eff_rmse = np.empty(namps)
        alphah_eff_rmse_trueQ = np.empty(namps)
        for i in range(namps):
            alphah_eff_rmse[i] = rmse(alphah_eff[i, :], alphah_eff_predict[i, :])
            alphah_eff_rmse_trueQ[i] = rmse(alphah_eff_trueQ[i, :], alphah_eff_predict_trueQ[i, :])
        ...
        if plot_bool == 1:
            ...
            for i in range(namps):
                ax.plot(Qm * 1e2, alphah_eff[i, :] * 1e-3, c=mymap(rescale(amps[i], Amin, Amax)))
            ...
            fig, ax = plt.subplots(figsize=(21, 7))
            ax.set_xlabel('$Q_m \ (nC/cm^2)$', fontsize=28)
            ax.set_ylabel('$\\alpha_{h,\ eff-sub}\ (ms^{-1})$', fontsize=28)
            ax.set_xlim(Qmin * 1e2, Qmax * 1e2)
            for i in range(namps):
                ax.plot(Qm * 1e2, alphah_eff_sub[i, :] * 1e-3,
            ...
            ax.set_ylabel('$\\alpha_{h,\ eff-sub-peaks}\ (ms^{-1})$', fontsize=28)
            ax.scatter(amps * 1e-3, alphah_eff_sub_peaks * 1e-3, s=30, c='C0', label='data')
            ax.plot(amps * 1e-3, alphah_eff_sub_peaks_fit * 1e-3, c='C1', label='fit')
            ...
            fig, ax = plt.subplots(figsize=(21, 7))
            ax.set_xlabel('$Q_m \ (nC/cm^2)$', fontsize=28)
            ax.set_ylabel('$\\alpha_{h,\ eff-sub-norm}\ (-)$', fontsize=28)
            ax.set_xlim(Qmin * 1e2, Qmax * 1e2)
            ax.grid()
            for i in range(namps - 1):
                ax.plot(Qm * 1e2, alphah_eff_sub_norm[i, :],
                        c=mymap(rescale(amps[i], Amin, Amax)))
            for i in range(namps - 1):
                ax.plot(Qm * 1e2, alphah_eff_sub_norm_fit[i, :], '--',
                        c=mymap(rescale(amps[i], Amin, Amax)))
            cbar = plt.colorbar(sm_amp)
            cbar.ax.set_ylabel('$A_{drive} \ (kPa)$', fontsize=28)
            plt.tight_layout()
            ...
            fig, ax = plt.subplots(figsize=(21, 7))
            ax.set_xlabel('$A_{drive}\ (kPa)$', fontsize=28)
            ax.set_ylabel('$\\alpha_{h,\ eff-sub-norm}\ fit\ params$', fontsize=28)
            ...
            # 7: alphah_eff_predict
            fig, ax = plt.subplots(figsize=(21, 7))
            ax.set_xlabel('$Q_m \ (nC/cm^2)$', fontsize=28)
            ax.set_ylabel('$\\alpha_{h,\ eff}\ prediction\ (ms^{-1})$', fontsize=28)
            ax.set_xlim(Qmin * 1e2, Qmax * 1e2)
            for i in range(namps):
                ax.plot(Qm * 1e2, alphah_eff_predict[i, :] * 1e-3,
                        c=mymap(rescale(amps[i], Amin, Amax)))
            cbar = plt.colorbar(sm_amp)
            cbar.ax.set_ylabel('$A_{drive} \ (kPa)$', fontsize=28)
            plt.tight_layout()
            ...
            # 8: alphah_eff_predict - alphah_eff
            fig, ax = plt.subplots(figsize=(21, 7))
            ax.set_xlabel('$Q_m \ (nC/cm^2)$', fontsize=28)
            ax.set_ylabel('$\\alpha_{h,\ eff}\ difference\ (ms^{-1})$', fontsize=28)
            ax.set_xlim(Qmin * 1e2, Qmax * 1e2)
            for i in range(namps):
                ax.plot(Qm * 1e2, alphah_eff_diff[i, :] * 1e-3,
                        c=mymap(rescale(amps[i], Amin, Amax)))
            cbar = plt.colorbar(sm_amp)
            cbar.ax.set_ylabel('$A_{drive} \ (kPa)$', fontsize=28)
            plt.tight_layout()
            ...
            ax.plot(amps * 1e-3, alphah_eff_rmse * 1e-3, c='C0',
                    label='$RMSE\ -\ entire\ Q_m\ range$')
            ax.plot(amps * 1e-3, alphah_eff_rmse_trueQ * 1e-3, c='C1',
                    label='$RMSE\ -\ realistic\ Q_m\ range$')
            ax.plot(amps * 1e-3, alphah_eff_maxdiff * 1e-3, '--', c='C0',
                    label='$MAE\ -\ entire\ Q_m\ range$')
            ax.plot(amps * 1e-3, alphah_eff_maxdiff_trueQ * 1e-3, '--', c='C1',
